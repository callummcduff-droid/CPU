...
char **read_assembly(int *num_lines, const char *filename){
    printf("Starting\n");
    char read_file[256];
    snprintf(read_file, sizeof(read_file), "%s.asm", filename);
    FILE *fptr = fopen(read_file, "r");
    printf("Opened write file\n");
    if (!fptr)
        return NULL;
...
void Branch_type(ParsedLine *parse, Assembled *assembled, Lookup key[], int size_key, int elmnt, int size_line) {
    ...
    const char *filename){
        char write_file [256];
        snprintf(write_file, sizeof(write_file), "%s.hex", filename);
        FILE *fptr = fopen(write_file, "w");
        printf("Opened write file\n");
